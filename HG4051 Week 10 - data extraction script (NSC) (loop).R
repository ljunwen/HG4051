...
search_string_series <- c("sia", "hor", "lah")

particles <- vector(mode = "list", length = 0)

for (p in search_string_series) {

# enter search string here

search_string <- paste0("\\[",p,"\\]")
...
particles <- c(particles, list(examples))
names(particles)[length(particles)] <- p
